...
parser.add_argument("--goggle_beta", default=2e-0, type=float,
...
parser.add_argument("--goggle_logging", default=1000, type=int,
...
parser.add_argument("--pred_model", default="MLP", type=str,
...
parser.add_argument("--LG_ff", default=0.9, type=float,
...
parser.add_argument("--LG_bfreq", default=30, type=int,
                    help="the bagging_freq of LightGBM.")
parser.add_argument("--LG_l1", default=0.5, type=float,
                    help="the lambda_l1 of LightGBM.")
parser.add_argument("--LG_l2", default=0.9, type=float,
                    help="the lambda_l2 of LightGBM.")
parser.add_argument("--is_linear_tree", default=True, type=bool,
                    help="if the model at leaf is a linear.")
parser.add_argument("--LG_lambda", default=0.1, type=float,
                    help="the linear_lambda of LightGBM.")
parser.add_argument("--LG_leaves", default=31, type=int,
                    help="the max num of LightGBM leaves.")

# FFTransformer & TabTransformer
parser.add_argument("--T_batch", default=128, type=int,
...
parser.add_argument("--T_attn_dropout", default=0.1, type=float,
...
def test_coda_data(test_dataloader, args):
    
    with open(os.path.join(save_dir, path_gen_test_data), 'rb') as f:
        X_train_synth, y_train_synth = pickle.load(f)
    ...
    X_train_synth = X_train_synth.to(device)
    y_train_synth = y_train_synth.to(device)
...
def main(arsgs):
    ...
    print(f"========== Test the model trained on CODA-generated data ==========")
    ...
    if args.pred_model == "MLP":
        print(f"=== pred_model: {args.pred_model}, MLP_lr: {args.MLP_lr} ===")
    elif args.pred_model == "LightGBM":
        print(f"=== pred_model: {args.pred_model}, LG_lr: {args.LG_lr}, LG_ff: {args.LG_ff}, LG_bfrac: {args.LG_bfrac}, LG_bfreq: {args.LG_bfreq}, LG_l1: {args.LG_l1}, LG_l2: {args.LG_l2}, LG_lambda: {args.LG_lambda}, LG_leaves: {args.LG_leaves} ===")
    elif args.pred_model == "FTTransformer":
        print(f"=== pred_model: {args.pred_model}, T_lr: {args.T_lr}, T_i_dim: {args.T_i_dim}, T_n_head: {args.T_n_head}, T_att_blocks: {args.T_att_blocks}, T_attn_dropout: {args.T_attn_dropout}, T_batch: {args.T_batch} ===")

    starting_time = time.time()

    test_coda_data(dataloaders[-1], args)
